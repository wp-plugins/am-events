			...
			if (isset($_POST['am_recurrent']))
			{
				...
				if ($recurrent === 'yes') { // If so, create the events
					...
					for ($i = 1; $i < $recurrent_amount; $i++) {
						...
						wp_set_post_tags($new_post_id, wp_get_post_tags($post_id, array('fields' => 'ids')), false);
			...
			if (!( wp_is_post_revision($post_id) && wp_is_post_autosave($post_id) )) {
				...
				if (isset($_POST['submit_all']) && $_POST['submit_all'] === 'yes') {
					...
					while ($the_query->have_posts()) {
						...
						wp_set_post_tags($id, wp_get_post_tags($post_id, array('fields' => 'ids')), false);
...
function am_register_post_type() {
	...
	register_taxonomy('post_tag', array('am_event'));
}
